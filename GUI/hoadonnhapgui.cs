...
namespace Doanqlchdt.GUI
{
    ...
    {
        static hoadonnhapbus bus = new hoadonnhapbus();
        ...
        public void loadlistview()
        {
            ...
            listView1.Columns.Add("Mã HDN", 100, HorizontalAlignment.Left); // Thêm các cột vào ListView
            listView1.Columns.Add("Mã NV", 150, HorizontalAlignment.Left);
            listView1.Columns.Add("Mã Nhà Cung Cấp", 150, HorizontalAlignment.Left);
            listView1.Columns.Add("Tổng Tiền", 200, HorizontalAlignment.Left);
        ...
        public void loadtable()
        {
            ...
            foreach (hoadonnhapdto hdndto in ds)
            {

                ListViewItem ls = new ListViewItem(hdndto.Mahdn);
                ls.SubItems.Add(hdndto.Manv);
                ls.SubItems.Add(hdndto.Ncc);
                ls.SubItems.Add(hdndto.Tongtien.ToString());
                ls.SubItems.Add(hdndto.Ngaytao.ToString("dd/MM/yyyy"));
        ...
        private void listView1_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            if (listView1.SelectedItems.Count > 0)
            {
                ...
                hoadonnhapdto hdndto = null;

                foreach (hoadonnhapdto hoadonndto in ds)
                {
                    if (hoadonndto.Mahdn == selectedMa)
                    {
                        hdndto = hoadonndto;
                    }
                }
                if (hdndto != null)
                {
                    ...
                    Chitietdonnhap ctdn = new Chitietdonnhap(hdndto);
                   ctdn.StartPosition = FormStartPosition.CenterParent;
                    ctdn.ShowDialog();
                    if (temppage == true)
                    {
                        loadtable();
